...
# ─────────────────────────────────────────────────────────────────────────────
# Build:  docker build -t british-cybersecurity-mcp .
# Run:    docker run --rm -p 3000:3000 british-cybersecurity-mcp
#
# The image bakes /app/data/ncsc.db at build time. Override NCSC_DB_PATH
...
ENV NCSC_DB_PATH=/app/data/ncsc.db
...
# Bake the pre-built database into the image so /app/data/ncsc.db resolves
...
# We then COPY it into the image at /app/data/ncsc.db (NCSC_DB_PATH). The
# explicit `data/<name>.db` reference is required for the workflow's grep
# `COPY\s+\K(data/\S+\.db)` to match.
COPY data/database.db data/ncsc.db
